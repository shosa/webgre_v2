...
try {
    ...
    $stmt = $conn->prepare("SELECT id, ragione_sociale , autorizzazione FROM exp_terzisti ORDER BY ragione_sociale ASC");
    $stmt->execute();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Recupera l'autorizzazione del terzista selezionato
        $stmt_terzista = $conn->prepare("SELECT autorizzazione FROM exp_terzisti WHERE id = :id_terzista");
        $stmt_terzista->bindParam(':id_terzista', $id_terzista, PDO::PARAM_INT);
        $stmt_terzista->execute();
        $terzista_data = $stmt_terzista->fetch(PDO::FETCH_ASSOC);
        $autorizzazione = $terzista_data['autorizzazione'];

        // Prepara l'inserimento
        $stmt = $conn->prepare("INSERT INTO exp_documenti (id, id_terzista, data, stato , autorizzazione) VALUES (:id, :id_terzista, :data, :stato, :autorizzazione)");
        $stmt->bindParam(':id', $newId, PDO::PARAM_INT);
        $stmt->bindParam(':id_terzista', $id_terzista, PDO::PARAM_INT);
        $stmt->bindParam(':data', $data);
        $stmt->bindParam(':stato', $stato);
        $stmt->bindParam(':autorizzazione', $autorizzazione);
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="alert alert-info">
                                <i class="fas fa-info-circle mr-2"></i> Stai per creare un nuovo documento di trasporto.
                                Seleziona il terzista dal menu a tendina e procedi allo step successivo.
                            </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-8">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control select2" id="terzista" name="terzista"
                                                        required>
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control bg-light" id="progressivo"
                                                        name="progressivo" value="<?php echo $newId; ?>" readonly>
                                                    <small class="form-text text-muted">Numero documento generato
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control bg-light" id="dataDoc"
                                                        name="dataDoc" value="<?php echo date('d/m/Y'); ?>" readonly>
