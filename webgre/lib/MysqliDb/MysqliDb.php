...
class MysqliDb
{
    ...
    protected function _buildInsertQuery($tableData)
    {
        ...
        $isInsert = preg_match('/^(INSERT|REPLACE)/', $this->_query); // Modificato il regex
        $dataColumns = array_keys($tableData);
        if ($isInsert) {
            if (!empty($dataColumns)) { // Controlla se ci sono colonne da inserire
    ...
    /*  CODICE ORIGINALE CHE HO MODIFICATO PER NON AVERE ERRORE DI IMPLODE ARRAY NULL
    protected function _buildInsertQuery($tableData)
    {
        if (!is_array($tableData)) {
            return;
