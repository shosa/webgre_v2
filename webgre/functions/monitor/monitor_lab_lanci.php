...
($operation == 'viewLab') ? $viewlab = true : $viewlab = false;
($operation == 'viewBrand') ? $viewBrand = true : $viewBrand = false;

// Users class
require_once BASE_PATH . '/lib/Lanci/Lanci.php';
$modelli = new Lanci();
if ($viewlab) {
    $laboratorio = filter_input(INPUT_GET, 'laboratorio', FILTER_UNSAFE_RAW);
    $pagelimit = 20;
    ...
    $db = getDbInstance();
    $labIDRow = $db->where('Nome', $laboratorio)->getOne('laboratori', 'ID');
    $labValue = $laboratorio;
    $labID = $labIDRow['ID'];
                        ...
                        if ($row['stato'] != 'IN ATTESA'): ?>
                            <tr>
                                ...
                                <td style="padding-top:30px;">
                                    ...
                                    echo '<div class="progress-bar progress-bar-success" role="progressbar" style="min-width: 5%; width: ' . $progressValue . '%;" aria-valuenow="' . $progressValue . '" aria-valuemin="0" aria-valuemax="100">';
                                    echo $progressValue . '%';
                                    echo '</div>';
                                    echo '</div>';
                                    ?>
                                ...
                                <td style="vertical-align: middle; text-align:center;">
                                    <a href="monitor_open_lab_lanci.php?lancio=<?php echo $row['lancio']; ?>&operation=plus"
...
if ($viewBrand) {
    $linea = filter_input(INPUT_GET, 'marchio', FILTER_UNSAFE_RAW);
    $pagelimit = 20;
    ...
    //Get DB instance. i.e instance of MYSQLiDB Library
    $db = getDbInstance();
    $lineaIDRow = $db->where('descrizione', $linea)->getOne('linee', 'sigla');
    $lineaID = $lineaIDRow['sigla'];
    $select = array('lanci.ID', 'lanci.taglio', 'lanci.preparazione', 'lanci.orlatura', 'lanci.spedizione', 'lanci.lancio', 'lanci.avanzamento', 'lanci.id_lab', 'SUM(lanci.paia) AS paia_total', 'lanci.id_modello', 'linee.descrizione', 'lanci.id_variante', 'lanci.id_lab', 'lanci.stato', 'lanci.paia', 'basi_modelli.descrizione AS modello_descrizione', 'var_modelli.desc_variante AS variante_descrizione');
    $db->groupBy('lanci.lancio');
    $db->join('basi_modelli', 'lanci.id_modello = basi_modelli.ID', 'LEFT');
    $db->join('linee', 'lanci.linea = linee.sigla', 'LEFT');
    $db->join('var_modelli', 'lanci.id_variante = var_modelli.ID', 'LEFT');
    $db->where('lanci.linea', $lineaID);
...
// Set pagination limit
    ...
    ?>
            ...
            <div id="page-wrapper">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header page-action-links text-left">Lanci in lavoro per
                            <b>
                                <?php echo $linea; ?>
                            </b>
                        </h1>
